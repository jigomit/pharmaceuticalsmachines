...
var Quality_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		const pillars = [
			...
			{
				title: "IQ / OQ",
				body: "Installation and Operational Qualification protocols supplied with every machine — ready for regulatory audit."
			},
			{
		...
		const services = [
			...
			{
				name: "IQ/OQ Validation Support",
				description: "Installation and Operational Qualification protocols supplied with every machine. Audit-ready documentation for regulatory submissions."
			},
			{
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(SeoHead_default, {
				title: "Quality & Compliance — cGMP, SS 316L, FAT/SAT, IQ/OQ",
				description: "cGMP design, SS 316L contact parts, FAT/SAT validation and IQ/OQ documentation on every pharmaceutical machine we ship.",
				schema: serviceSchema.value
			}, null, _parent));
			...
			_push(`<!--]--></div><div class="mt-10 flex flex-wrap gap-3">`);
			_push(ssrRenderComponent(unref(Link), {
				href: "/products/injectable-liquid-filling/automatic-vial-filling-machine",
				class: "inline-flex items-center rounded-full bg-[color:var(--color-brand)] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[color:var(--color-brand-accent)]"
			}, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(` Vial filling machine specs `);
					else return [createTextVNode(" Vial filling machine specs ")];
				}),
				_: 1
			}, _parent));
			_push(ssrRenderComponent(unref(Link), {
				href: "/products/washing-machines/linear-ampoule-washing-machine",
				class: "inline-flex items-center rounded-full border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-700 hover:border-[color:var(--color-brand-accent)] hover:text-[color:var(--color-brand-accent)]"
			}, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(` Ampoule washing line `);
					else return [createTextVNode(" Ampoule washing line ")];
				}),
				_: 1
			}, _parent));
